...
const CELL_NAMES = {
    ...
    3: ['Left third', 'Middle third', 'Right third'],
    4: ['First quarter', 'Second quarter', 'Third quarter', 'Fourth quarter'],
};

const SPAN_NAMES = new Map([
    ['1/2', 'half'], ['1/3', 'third'], ['2/3', 'two-thirds'],
    ['1/4', 'quarter'], ['3/4', 'three-quarters'],
]);
...
/**
 * A human-readable name for a parsed spec, for use where a drawing cannot go.
 * Falls back to the spec itself for grids that have no natural name.
 */
export function describeSpec(spec) {
    if (!spec)
        return 'Empty';
    const {cols, rows, x1, y1, x2, y2} = spec;
    const raw = `${cols}x${rows} ${x1}:${y1} ${x2}:${y2}`;
    if (rows !== 1)
        return raw;
    ...
    if (span === cols)
        return 'Full screen';
    if (span === 1 && CELL_NAMES[cols])
        return CELL_NAMES[cols][x1 - 1];

    const divisor = gcd(span, cols);
    const unit = SPAN_NAMES.get(`${span / divisor}/${cols / divisor}`);
    if (!unit)
        return raw;
    if (x1 === 1)
        return `Left ${unit}`;
