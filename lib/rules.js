...
// Rule shape: { app: string, workspace: number|null (1-based), preset: number (1-based slot),
//               presets?: number[] (ordered slots for sequenced placement), enabled: boolean }
...
export function validateRule(raw) {
    ...
    if (raw.presets !== undefined && raw.presets !== null) {
        if (!Array.isArray(raw.presets) || raw.presets.length === 0)
            return null;
        if (raw.presets.some(n => !Number.isInteger(n) || n < 1 || n > PRESET_SLOTS))
            return null;
        presets = [...raw.presets];
    }
    const preset = raw.preset === undefined && presets ? presets[0] : raw.preset;
    if (!Number.isInteger(preset) || preset < 1 || preset > PRESET_SLOTS)
        return null;
    const enabled = raw.enabled === undefined ? true : Boolean(raw.enabled);
    const rule = {app, workspace, preset, enabled};
    if (presets)
        rule.presets = presets;
    return rule;
}
